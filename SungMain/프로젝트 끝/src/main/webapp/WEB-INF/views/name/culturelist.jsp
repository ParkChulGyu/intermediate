...
<link href="https://getbootstrap.com/docs/5.3/assets/css/docs.css"
	rel="stylesheet">
<title>name/culturelist.jsp</title>
<script
...
<body>
	...
	<main>
		<h1>culturelist</h1>
		<h4>
			<form action="/web/name/culturelist" method="post">
			...
			<c:forEach var="cn" items="${cnlist}" varStatus="statusNm">
				<c:choose>
			...
			<ul class="pagination">
				<c:if test="${pdto.isPrev eq true }">
					<li class="page-btn"><a
						href="/web/name/culturelist?pageNum=${pdto.pageNum - 1}&str=${str}&listNum=${listNum}">이전</a>
					</li>
				</c:if>
				<c:forEach begin="${pdto.startPage}" end="${pdto.endPage}"
					...
					<c:choose>
						...
						<c:otherwise>
							<li class="page-btn"><a
								href="/web/name/culturelist?pageNum=${pageNumber}&str=${str}&listNum=${listNum}">${pageNumber}</a>
							</li>
				...
				<c:if test="${pdto.isNext eq true }">
					<li class="page-btn"><a
						href="/web/name/culturelist?pageNum=${pdto.pageNum + 1}&str=${str}&listNum=${listNum}">다음</a>
					</li>
				</c:if>
			</ul>
