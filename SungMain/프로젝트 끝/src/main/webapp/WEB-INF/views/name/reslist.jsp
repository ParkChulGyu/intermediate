...
<body>
	...
	<main>
		<h1>reslist</h1>
		<h4>
			<form action="/web/name/reslist" method="post">
				...
				<div>
				 <input type="text" name="str" class="form-write" placeholder="검색어를 입력하세요">
			...
			<c:forEach var="rn" items="${rnlist}" varStatus="statusNm">
				<c:choose>
			...
			<ul class="pagination">
				<c:if test="${pdto.isPrev eq true }">
					<li class="page-btn"><a
						href="/web/name/reslist?pageNum=${pdto.pageNum - 1}&str=${str}&listNum=${listNum}">이전</a>
					</li>
				</c:if>
				<c:forEach begin="${pdto.startPage}" end="${pdto.endPage}"
					...
					<c:choose>
						...
						<c:otherwise>
							<li class="page-btn"><a
								href="/web/name/reslist?pageNum=${pageNumber}&str=${str}&listNum=${listNum}">${pageNumber}</a>
							</li>
				...
				<c:if test="${pdto.isNext eq true }">
					<li class="page-btn"><a
						href="/web/name/reslist?pageNum=${pdto.pageNum + 1}&str=${str}&listNum=${listNum}">다음</a>
					</li>
				</c:if>
			</ul>
