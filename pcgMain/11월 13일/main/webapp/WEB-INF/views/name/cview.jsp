				...
				<div class="text-tabmenu out-tabmenu">
					<ul>
						<li id="tab1" class="btnCon"><input type="radio" checked
							...
							<div class="tabCon">
								<table class="table-tab">
									<c:choose>
										<c:when test="${not empty cdto.post_num}">
										<tr>
										<th class="jb-th">우편번호</th>
										<td class="jb-td">${cdto.post_num}</td>
										</tr>
										</c:when>
									</c:choose>
										<c:choose>
										<c:when test="${not empty cdto.address}">
										<tr>
										<th class="jb-th">주소</th>
										<td class="jb-td">${cdto.address}</td>
										</tr>
										</c:when>
									</c:choose>
										<c:choose>
										<c:when test="${not empty cdto.information}">
										<tr>
										<th class="jb-th">상세 정보</th>
										<td class="jb-td">${cdto.information}</td>
										</tr>
										</c:when>
									</c:choose>
								</table>
						...
						<li id="tab2" class="btnCon"><input type="radio"
							...
							<table class="table-tab">
									<c:choose>
										<c:when test="${not empty cdto.use_time}">
										<tr>
										<th class="jb-th">이용 시간</th>
										<td class="jb-td">${cdto.use_time}</td>
										</tr>
										</c:when>
									</c:choose>
										<c:choose>
										<c:when test="${not empty cdto.day_off}">
										<tr>
										<th class="jb-th">쉬는 날</th>
										<td class="jb-td">${cdto.day_off}</td>
										</tr>
										</c:when>
									</c:choose>
								</table>
						...
						<li id="tab3" class="btnCon"><input type="radio"
							...
							<table class="table-tab">
									<c:choose>
										<c:when test="${not empty cdto.scale}">
										<tr>
										<th class="jb-th"> 규 모 </th>
										<td class="jb-td">${cdto.scale}</td>
										</tr>
										</c:when>
									</c:choose>
										<c:choose>
										<c:when test="${not empty cdto.capacity}">
										<tr>
										<th class="jb-th">이용 가능 인원</th>
										<td class="jb-td">${cdto.capacity}</td>
										</tr>
										</c:when>
									</c:choose>
										<c:choose>
										<c:when test="${not empty cdto.parking}">
										<tr>
										<th class="jb-th">주차 시설</th>
										<td class="jb-td">${cdto.parking}</td>
										</tr>
										</c:when>
									</c:choose>
										<c:choose>
										<c:when test="${not empty cdto.pets}">
										<tr>
										<th class="jb-th">반려 동물 동반 여부</th>
										<td class="jb-td">${cdto.pets}</td>
										</tr>
										</c:when>
									</c:choose>
								</table>
